...
void InnerProductAndDropoutLayer<Dtype, MItype, MOtype>::LayerSetUp(
  ...
  transpose_ = this->layer_param_.inner_product_and_dropout_param().transpose();
  N_ = num_output;
  const int_tp axis = bottom[0]->CanonicalAxisIndex(
      this->layer_param_.inner_product_and_dropout_param().axis());
  ...
  if (this->blobs_.size() > 0) {
    ...
    if (is_float_type<Dtype>()) {
      shared_ptr<Filler<Dtype> > weight_filler(GetFiller<Dtype>(
            this->layer_param_.inner_product_and_dropout_param().weight_filler()));
    ...
    if (bias_term_) {
      ...
      if (is_float_type<Dtype>()) {
        shared_ptr<Filler<Dtype> > bias_filler(GetFiller<Dtype>(
                this->layer_param_.inner_product_and_dropout_param().bias_filler()));
...
template<typename Dtype, typename MItype, typename MOtype>
void InnerProductAndDropoutLayer<Dtype, MItype, MOtype>::Reshape(
    const vector<Blob<MItype>*>& bottom,
    const vector<Blob<MOtype>*>& top) {
...
template<typename Dtype, typename MItype, typename MOtype>
void InnerProductAndDropoutLayer<Dtype, MItype, MOtype>::Forward_cpu(
    const vector<Blob<MItype>*>& bottom,
    const vector<Blob<MOtype>*>& top) {
...
#ifdef CPU_ONLY
STUB_GPU(InnerProductAndDropoutLayer);
#endif

INSTANTIATE_CLASS_3T_GUARDED(InnerProductAndDropoutLayer,
...
REGISTER_LAYER_CLASS(InnerProductAndDropout);
REGISTER_LAYER_CLASS_INST(InnerProductAndDropout, (half_fp), (half_fp), (half_fp));
REGISTER_LAYER_CLASS_INST(InnerProductAndDropout, (float), (float), (float));
REGISTER_LAYER_CLASS_INST(InnerProductAndDropout, (double), (double), (double));
REGISTER_LAYER_CLASS_INST(InnerProductAndDropout, (uint8_t), (uint8_t), (uint8_t));
REGISTER_LAYER_CLASS_INST(InnerProductAndDropout, (uint16_t), (uint16_t), (uint16_t));
REGISTER_LAYER_CLASS_INST(InnerProductAndDropout, (uint32_t), (uint32_t), (uint32_t));
REGISTER_LAYER_CLASS_INST(InnerProductAndDropout, (uint64_t), (uint64_t), (uint64_t));
